...
#| Question #1:
   ...
   transformation function.  Doing this over already-parsed concrete
   ...
   See the definition of global-transformers below.
|#
...
;; Reminders:
;; * the transformers are a (Listof (List Symbol (Syntax -> Syntax)))
;; * and we have
;;   make-transformer : (Listof Symbol) (Listof (List Sexpr Sexpr))
;;                      -> (Sexpr -> Sexpr)

(: named-transformer : Symbol (Listof Symbol) (Listof String)
                       -> (List Symbol (Sexpr -> Sexpr)))
...
(define (named-transformer name keywords templates)
  (: even-list->list-of-2lists :
     (All (A) (Listof A) -> (Listof (List A A))))
...
(define (execute-unref val receiver)
  (let ([r (cases val [(RktV x) (and (ref? x) x)] [else #f])])
    (if r
      ...
      (error 'print "cannot `unref' a non-ref value: ~s" val))))
...
(define (execute-setref val newval)
  (let ([r (cases val [(RktV x) (and (ref? x) x)] [else #f])])
    (if r
      (set-ref! r newval)
      (error 'print "cannot `set-ref!' a non-ref value: ~s" val))))
...
(define (execute-val val)
  (let* ([val (strict val)]
         [io  (cases val
                ...
                [else #f])])
    (if (not io)
      (error 'execute-io "expecting an IO value: ~s" val)
      (cases io
...
;; The global transformers were not used in the above examples since
;; they were always overwritten by a local `with-stx'.  Now, here are
;; some examples that show how these definitions work:
...
;; something for complete coverage
(test (named-transformer 'let '() '("foo"))
      =error> "odd number of templates")
