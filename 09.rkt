...
Core evaluation rules:
  ...
  eval({call E1 E2},env1)    = eval(Ef,cons(eval(E2,env1),env2))
                             ...
                             = error!  otherwise
  ;; The rule for `rec' is a rewrite rule:
  eval({rec {foo E1} E2}) =
    = eval({with {foo {call Y {fun {foo} E1}}} E2})
    where Y = {fun {g}
                {{fun {x} {call x x}}
                 {fun {x} {call g {fun {z} {call {call x x} z}}}}}}
  eval({if0 COND E1 E2}, env) = eval(E1, env) if eval(COND, env) == 0
     ...
     (match sexpr
       [(list 'rec (list (symbol: name) named) body)
        (let ([named (parse-sexpr named)])
          (cases named
            [(Fun id expr) (WRec name named (parse-sexpr body))]
            [else (error 'parse-sexpr "non-fun form in `rec': ~s"
                         sexpr)]))]
...
(: Y-combinator : BRANG)
;; The precompiled value for the Y combinator.
(define Y-combinator
  (parse "{fun {g}
            {call {fun {x} {call x x}}
                  {fun {x}
                    {call g {fun {z} {call {call x x} z}}}}}}"))
...
(define (preprocess expr de-env)
  ...
  (cases expr
    ...
    [(If0 c t e) (CIf0 (sub c) (sub t) (sub e))]
    [(With bound-id named-expr bound-body)
     (sub (Call (Fun (list bound-id) bound-body) (list named-expr)))]
    [(WRec bound-id named-expr bound-body)
     (sub (With bound-id
                (Call Y-combinator
                      (list (Fun (list bound-id) named-expr)))
                bound-body))
     #| Alternatively, you could define `Y-combinator' with
          (define Y-combinator
            (preprocess (parse "...") de-empty-env))
        This works with de-empty-env because it has no free identifiers.
        The problem with that is that you get a CORE which you will not
        be able to stick into a BRANG structure, and that means that you
        need to use it inside a CORE, which in turn means that you need
        to effectively replicate the preprocessing job of the `With' and
        `Fun' branches in this case.  The resulting code is therefore
        more complicated:
          [(WRec bound-id named-expr bound-body)
           (With* (Call* Y-combinator
                         (Fun* (preprocess named-expr
                                           (de-extend de-env
                                                      bound-id))))
                  (preprocess bound-body
                              (de-extend de-env bound-id)))]
        and since it achieves a little performance in compile-time,
...
(define (eval expr env)
  (cases expr
    ...
    [(CIf0 c t e)
     ;; Remember that it's easier to read such deeply nested
     ;; expressions from the inside out: so this will evaluate `c',
     ;; then inspect the result and return #t only if it was a 0
     ;; number, then based on that use either `t' or `e' for the rest
     ;; of the evaluation.
     ;; (eval (if (cases (eval c env) [(NumV n) (zero? n)] [else #f])
     ;;         t
     ;;         e)
     ;;       env)
     ;; Shorter alternative, uses the fact that we can use `equal?'
     ;; with VALs:
     (eval (if (equal? (eval c env) (NumV 0)) t e) env)]
...
(test (run "{rec x}") =error> "bad `rec' syntax")
(test (run "{if0 1}") =error> "bad `if0' syntax")
(test (run "{rec {x 1} 2}") =error> "non-fun form in `rec'")
