  ...
  (lambda (n cols)
    (if (zero? n)
      ...
      (append* (map (lambda (rest)
                      (map (lambda (new) (cons new rest))
                           (filter (lambda (new) (safe? new 1 rest))
                                   cols)))
...
;; example for defining the time spent
(define hours-spent (+ 3 3))
(test (->nat hours-spent) => '6) ; force the expression
